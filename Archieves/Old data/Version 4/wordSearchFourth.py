        ...
        if type(pdfLinks[i]) is not float:   
            ...
            try:
                if wordSearch(pdfLinks[i], wordList) == "Present":
                    ...
                    rowError+=1
                elif wordSearch(pdfLinks[i], wordList) == "Nothing":
...
def wordSearch(url, target: [str]):
    ...
    for i in range(num_pages):
        ...
        if text == "":  #meaning that the page is blank == Image
            return("Image")
            #return imageSearch(res, target)  #Assume that if the first page is an image, the whole report is in image format, so the function marks it as an image
